module "mysql" {
  ...
  domain = var.domain
  vpc_security_group_ids = vpc_security_group_ids = var.components["mysql"]
  Name = "mysql"
  env = var.env
  instance_type = ""
...
module "backend" {
  ...
  zone_id = data.aws_route53_zone.main.zone_id
  domain = var.domain
  vpc_security_group_ids = var.components["backend"]
  Name = "backend"
  env = var.env
  instance_type = ""
...
module "frontend" {
  ...
  zone_id = data.aws_route53_zone.main.zone_id
  domain = var.domain
  vpc_security_group_ids = vpc_security_group_ids = var.components["frontend"]
  Name = "frontend"
  env = var.env
  instance_type = ""
